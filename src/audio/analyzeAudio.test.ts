// Tests commented out — Path B (neighbor-relative comparison) changed shouldVibrate to computeVibrationMap
// Old tests were for absolute threshold + RMS diff bounds approach
// TODO: write new tests for computeVibrationMap once approach is validated
...
//     expect(t.endTime).toBe(9 / 100)
//     expect(t.min).toBe(0.1)
//     expect(t.max).toBe(1.0)
//   })
//
...
//     const trends = computeTrends(data, 1000, 10)
//     expect(trends[0].startTime).toBe(0)
//     expect(trends[0].endTime).toBe(9 / 1000)
//     expect(trends[1].startTime).toBe(10 / 1000)
//     expect(trends[1].endTime).toBe(19 / 1000)
//   })
// })
//
// // ── trendsToVibrationPattern ──
...
//     ]
//     const pattern = trendsToVibrationPattern(trends)
//     expect(pattern).toEqual([0, 120])
//   })
//
//   test('alternating vibrate/pause pattern', () => {
//     const trends = [
//       makeTrend({startTime: 0, endTime: 0.06, max: 0.8, leftRms: 0.3, rightRms: 0.3}),
//       makeTrend({startTime: 0.06, endTime: 0.12, max: 0.1}),
//       makeTrend({startTime: 0.12, endTime: 0.18, max: 0.9, leftRms: 0.4, rightRms: 0.4}),
//     ]
//     const pattern = trendsToVibrationPattern(trends)
//     expect(pattern).toEqual([60, 60, 60])
//   })
//
//   test('leading pause gets 0 prepended', () => {
//     const trends = [
//       makeTrend({startTime: 0, endTime: 0.06, max: 0.1}),
//       makeTrend({startTime: 0.06, endTime: 0.12, max: 0.8, leftRms: 0.3, rightRms: 0.3}),
//     ]
//     const pattern = trendsToVibrationPattern(trends)
